...
def test_terminal_reconciliation_is_idempotent() -> None:
    supervisor, _, resources = _supervisor()
    first = supervisor.run()
    terminal_count = resources.terminal_count

    second = supervisor._reconcile_terminal(0, 0)

    assert second is first
    assert resources.terminal_count == terminal_count


def test_primary_resource_failure_retains_secondary_terminal_limitations() -> None:
    channel = _Channel((_phase("started", 1),))
    process = _LiveUntilSignalProcess(channel)
    resources = _Resources(live_failure=True, terminal_failure=True)
    ...
    supervisor, _, _ = _supervisor(
        ...
        process=process,
        resources=resources,
        backend=_Backend(quiescence_failure=True),
        terminal_reader=fail_receipt,
    )
    ...
    assert ControlFailureCode.TERMINAL_RESOURCE_UNAVAILABLE in result.secondary_failures
    assert ControlFailureCode.EVENT_TRANSPORT_EOF in result.secondary_failures
    assert ControlFailureCode.BACKEND_QUIESCENCE_TIMEOUT in result.secondary_failures
    assert ControlFailureCode.RECEIPT_READBACK_FAILED in result.secondary_failures
    assert result.signal_count == 1


def test_resource_close_failure_does_not_skip_backend_or_readback() -> None:
    class CloseFailingResources(_Resources):
        def close(self):
            raise ControlFailure(ControlFailureCode.RESOURCE_READER_UNAVAILABLE)
    ...
    class RecordingBackend(_Backend):
        ...
        def wait_quiescent(self, timeout_seconds):
            self.wait_count += 1
            return super().wait_quiescent(timeout_seconds)
    ...
    supervisor, _, _ = _supervisor(
        resources=CloseFailingResources(),
        backend=backend,
        terminal_reader=readback,
    )

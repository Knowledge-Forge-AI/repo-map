...
def test_each_injected_primary_precedes_later_event_eof(primary) -> None:
    channel = _Channel(())
    process = _Process(channel)
    supervisor, _, _ = _supervisor(channel=channel, process=process)
    supervisor._record_failure(primary)

    result = supervisor.run()

    assert result.primary_control_failure is primary
    assert ControlFailureCode.EVENT_TRANSPORT_EOF in result.secondary_failures


def test_startup_receiver_eof_is_secondary_to_existing_primary() -> None:
    ...
    supervisor._record_failure(ControlFailureCode.AMBIENT_CLIENT_DETECTED)
...
def test_ignored_signal_is_secondary_to_the_triggering_control_failure() -> None:
    channel = _Channel((_phase("started", 1),))
    process = _Process(channel, never_exits=True)
    supervisor, _, _ = _supervisor(
        channel=channel,
        process=process,
        resources=_Resources(live_failure=True),
        clock=_Clock(step=2_000_000_000),
    )
    ...
    assert ControlFailureCode.CHILD_IGNORED_SIGNAL in result.secondary_failures
    assert result.signal_count == 1
    assert result.child_quiescent is False

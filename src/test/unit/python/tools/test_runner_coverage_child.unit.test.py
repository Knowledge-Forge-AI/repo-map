...
class ChildCoverageSessionUnitTests(ChildCoverageTestCase):
    ...
    def test_normal_child_return_measures_and_combines_coverage(self):
        ...
        with runner_coverage.ChildCoverageSession(
            coverage_module=coverage, scratch_dir=self.tmpdir / "session", source_root=source_dir,
        ) as session:
    ...
    def test_nonzero_return_preserves_primary_failure(self):
        ...
        with patch.object(run_tests, "run_pytest", side_effect=fake_run_pytest_failing):
            exit_code, runner = run_tests.run_pytest_with_coverage(
                coverage, dummy_pytest, [], dummy_plugin, scratch_dir=self.tmpdir / "fail_session",
            )
        ...
        with self.assertRaisesRegex(RuntimeError, "primary assertion failure"):
            with patch.object(run_tests, "run_pytest", side_effect=fake_run_pytest_raising):
                run_tests.run_pytest_with_coverage(
                    coverage, dummy_pytest, [], dummy_plugin, scratch_dir=self.tmpdir / "raise_session",
                )
    ...
    def test_sequential_attempt_isolation_purges_stale_shards(self):
        ...
        with runner_coverage.ChildCoverageSession(
            coverage_module=coverage, scratch_dir=session_scratch, source_root=self.tmpdir,
        ):
    ...
    def test_source_copy_mapping_remaps_to_canonical_source(self):
        ...
        with runner_coverage.ChildCoverageSession(
            coverage_module=coverage, scratch_dir=self.tmpdir / "mapped_session",
            source_root=canonical_dir, source_paths=[copied_dir],
        ) as session:
            runner = session.create_coverage(coverage)
            runner.start()
            try:
    ...
    def test_scratch_dir_none_allows_reporting_after_context_exit(self):
        ...
        session = runner_coverage.ChildCoverageSession(
            coverage_module=coverage, scratch_dir=None, source_root=source_dir,
        )
        with session:
            runner = session.create_coverage(coverage)
            runner.start()
    ...
    def test_canonical_mapping_preserves_line_and_branch_coverage(self):
        ...
        with runner_coverage.ChildCoverageSession(
            coverage_module=coverage, scratch_dir=self.tmpdir / "branch_session",
            source_root=canonical_dir, source_paths=[copied_dir],
        ) as session:
            runner = session.create_coverage(coverage)
            runner.start()
    ...
    def test_non_identical_copied_source_rejected(self):
        ...
        session = runner_coverage.ChildCoverageSession(
            coverage_module=coverage, scratch_dir=self.tmpdir / "reject_session",
            source_root=canonical_dir, source_paths=[mismatched_dir],
        )
        ...
        self.assertFalse(session._verify_source_identity(mismatched_dir))
        self.assertEqual(
            session.source_rejections[-1],
            f"{diff_size_file}: file mismatch with {canonical_file}",
        )
    ...
    def test_empty_or_corrupt_shard_raises_during_combine(self):
        session = runner_coverage.ChildCoverageSession(
            coverage_module=coverage, scratch_dir=self.tmpdir / "corrupt_session", source_root=self.tmpdir,
        )
        with session:
            runner = session.create_coverage(coverage)
        ...
        for primary_outcome in ["success", "nonzero", "exception"]:
            with self.subTest(primary_outcome=primary_outcome):
                session = runner_coverage.ChildCoverageSession(
                    ...
                    scratch_dir=self.tmpdir / f"matrix_session_{primary_outcome}",
                    ...
                    with patch.object(run_tests, "run_pytest", side_effect=fake_run):
                        if primary_outcome == "exception":
                            with self.assertRaisesRegex(RuntimeError, "primary suite crash"):
                                run_tests.run_pytest_with_coverage(
                                    coverage, dummy_pytest, [], dummy_plugin, session=session,
                                )
                        else:
                            exit_code, runner = run_tests.run_pytest_with_coverage(
                                coverage, dummy_pytest, [], dummy_plugin, session=session,

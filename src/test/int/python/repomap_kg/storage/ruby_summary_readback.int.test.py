...
COUNT_FIELDS = EXPECTED_FIELDS[2:18]


def test_psycopg65_ruby_summary_connector_and_cli_parity() -> None:
    require_postgres_binaries()
    root_path = "/tmp/psycopg65-ruby-public"
    repository_name = "psycopg65-ruby-public"
    missing_root = "/tmp/psycopg65-ruby-missing"
    invalid_psql = "/bin/psql-not-used-by-psycopg65"
    ...
    try:
        with temporary_postgres() as postgres:
            ...
            for mode, selector, connector, psql_command in modes:
                ...
                jsonable_payloads[mode] = ruby_summary_to_jsonable(record)
            ...
            for mode, selector, connector, psql_command in (
                ...
                _select_connector(selector, connector, postgres=postgres)
                missing_payloads[mode] = query_ruby_summary(
                    postgres.psql_args,
            ...
            for forbidden in (
                ...
                "https://example.invalid/rubygems",
                "EXAMPLE_API_KEY",
...
def _load_fixture(*, root_path: str, repository_name: str, postgres) -> None:
    result = run_repo_map_in_process(
        "storage",
        "load-files",
        str(canonicalization_fixture("ruby_basic", "raw_observations.jsonl")),
        "--repository-name",
        repository_name,
...
def _assert_nonempty_contract(payload: dict[str, object]) -> None:
    ...
    assert _count(payload, "ruby_files") >= 10
    assert _count(payload, "routes") >= 2
    assert _count(payload, "test_methods") >= 1
    assert _count(payload, "gem_dependencies") >= 4
    assert _count(payload, "vagrant_configs") >= 5
    assert _count(payload, "rake_tasks") >= 2
    profile_counts = payload["profile_counts"]
    assert isinstance(profile_counts, dict)
    assert len(profile_counts) >= 2

...
class StorageSourceRunRecordsPsycopgParityIntegrationTests(unittest.TestCase):
    def test_psycopg35_source_run_records_psycopg_driver_parity(self):
        require_postgres_binaries()
        ...
        from repomap_kg.server.mcp import repomap_source_runs
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                with temporary_postgres() as postgres:
                    ...
                    psql_records = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="example-rss-feed",
                        psql_command=postgres.psql_command,
                    )
                    psql_limit_one = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="example-rss-feed",
                        limit=1,
                    ...
                    psql_missing_source = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="missing-source",
                        psql_command=postgres.psql_command,
                    )
                    psql_mcp_payload = repomap_source_runs(
                        **mcp_args,
                        source_id="example-rss-feed",
                        limit=1,
                    )
                    ...
                    default_records = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="example-rss-feed",
                        psql_command=psycopg_only_psql_command,
                    )
                    default_limit_one = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="example-rss-feed",
                        limit=1,
                    ...
                    default_missing_source = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="missing-source",
                        psql_command=psycopg_only_psql_command,
                    ...
                    psycopg_records = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="example-rss-feed",
                        psql_command=psycopg_only_psql_command,
                    )
                    psycopg_limit_one = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="example-rss-feed",
                        limit=1,
                    ...
                    psycopg_missing_source = query_source_run_records(
                        postgres.psql_args,
                        root_path=str(root_path),
                        source_id="missing-source",
                        psql_command=psycopg_only_psql_command,
                    )
                    psycopg_mcp_payload = repomap_source_runs(
                        **mcp_args,
                        source_id="example-rss-feed",
                        limit=1,
                    )
        ...
        self.assertEqual(
            [record["source_run_id"] for record in psql_payload],
            ["20260630T120000Z"],
        )
        self.assertEqual(
            [record["source_run_id"] for record in psql_payload],
        ...
        self.assertIsInstance(record["acquired_at"], str)
        self.assertIsInstance(record["artifact_id"], str)
        self.assertIsInstance(record["artifact_path"], str)
        self.assertIsInstance(record["artifact_byte_length"], int)
        self.assertIsInstance(record["artifact_sha256"], str)
        self.assertEqual(len(record["artifact_sha256"]), 64)
        self.assertTrue(
            all(char in "0123456789abcdef" for char in record["artifact_sha256"])
        )
        self.assertEqual(record["http_status"], 200)
        self.assertEqual(record["content_type"], "application/rss+xml")
        self.assertGreaterEqual(record["observation_count"], 1)
        self.assertEqual(record["status_summary"], "ok")

        serialized = json.dumps(
            {
                "runs": psql_payload,
                "mcp_runs": psycopg_mcp_payload,

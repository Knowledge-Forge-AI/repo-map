"""Maintained complete optional-import owner context; parsed but never executed."""
SOURCE = r'''
...
from scale12_resource_sampling import ResourceSamplingError, read_process_rss_bytes, read_process_tree_rss_bytes

class _Process:

    def __init__(self, rss: object, *, children: tuple[_Process, ...]=(), running: bool=True, memory_error: BaseException | None=None, children_error: BaseException | None=None) -> None:
        self._rss = rss
...
def test_process_rss_returns_current_bytes_without_a_host_process(monkeypatch: pytest.MonkeyPatch) -> None:
    _install_process(monkeypatch, _Process(123456))
    boundary = RecordingProcessBoundary().install(monkeypatch)
    assert read_process_rss_bytes(17) == 123456
...
def test_process_rss_rejects_invalid_process_identifiers(value: object) -> None:
    with pytest.raises(ResourceSamplingError, match='process identifier'):
        read_process_rss_bytes(value)

@pytest.mark.parametrize('error', [psutil.NoSuchProcess(17), psutil.AccessDenied(17), psutil.ZombieProcess(17)])
def test_process_rss_maps_owned_psutil_failures_to_none(monkeypatch: pytest.MonkeyPatch, error: BaseException) -> None:
...
def test_process_rss_refuses_invalid_values(monkeypatch: pytest.MonkeyPatch, rss: object) -> None:
    _install_process(monkeypatch, _Process(rss))
    assert read_process_rss_bytes(17) is None

def test_process_tree_sums_root_and_available_children_without_ps(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    root = _Process(100, children=(_Process(10), vanished, denied, zombie, reused))
...
@pytest.mark.parametrize('error', [psutil.NoSuchProcess(17), psutil.AccessDenied(17), psutil.ZombieProcess(17)])
def test_process_tree_root_disappearance_or_denial_returns_none(monkeypatch: pytest.MonkeyPatch, error: BaseException) -> None:
    _install_process(monkeypatch, _Process(100, children_error=error))
    assert read_process_tree_rss_bytes(17) is None

def test_process_tree_refuses_reused_root_pid(monkeypatch: pytest.MonkeyPatch) -> None:

#!/usr/bin/env python3
"""Run the repeatable native Windows evidence slice for ASYNC14."""
...
TESTS = (
    "src/test/int/python/repomap_kg/coordinator/windows_runtime.int.test.py",
)
...
def main() -> int:
    if sys.platform == "win32":
        ...
        print("service_manager=not-mutated;foreground-and-transport-only")
        # The repository integration conftest starts a Linux Postgres container at
        # session startup. This proof slice is deliberately transport/process-only;
        # disable all conftests so a Windows Docker engine is not mistaken for a
        # configured PostgreSQL runtime.
        command = [sys.executable, "-m", "pytest", "--noconftest", "-q", *TESTS]
        completed = subprocess.run(command, cwd=REPO_ROOT, check=False)
        return completed.returncode
    else:
        print("ASYNC14 native Windows tests require a Windows host", file=sys.stderr)
        return 2
